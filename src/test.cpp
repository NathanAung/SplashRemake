...
class Player{
	private:
		...
		Vec2 m_cameraOffset;	// カメラの座標補正値
		float m_moveForce;		// 移動速度
		float m_jumpForce;		// ジャンプ時に加わる力
	...
	public:
		...
		/// @param cameraOffset 
		/// @param cameraRatio 
		Player(P2World* world, Image image, Vec2 firstPos, float moveForce, float jumpForce, Vec2 cameraOffset, double cameraRatio){
			// m_state = aa
			m_world = world;
			...
			MultiPolygon outLine = image.alphaToPolygons(0);
			Polygon shape = outLine[0];
			shape = shape.scaled(0.5f);
			m_camera = { firstPos + m_cameraOffset, cameraRatio };
			m_collider = m_world->createPolygon(P2Dynamic, firstPos, shape, P2Material{ 1.0f, 0, 0, 1.0f});
...
void Main()
{
	...
	constexpr Vec2 m_cameraOffset{ 0, -300 };

	constexpr float m_moveForce = 150;

	constexpr float m_jumpForce = 1000;

	Image image(U"🐈"_emoji);

	Player m_player(&world, image, m_firstPos, m_moveForce, m_jumpForce, m_cameraOffset, 0.5);
	// 地面
